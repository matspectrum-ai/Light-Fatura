...
func (s *Service) createReserved(ctx context.Context, reservation RequestReservation, request CreateRequest) (*Transaction, error) {
	...
	for _, record := range order {
		...
		if record.DailyLimit != nil && *record.DailyLimit > 0 {
			...
			count, e := s.store.GatewayTransactionsSince(ctx, record.Slug, start)
			if e != nil {
				s.log(ctx, LogEntry{GatewaySlug: record.Slug, InvoiceID: &request.InvoiceID, Level: "erro", Message: e.Error()})
				continue
			}
		...
		created, e := adapter.CreatePIX(ctx, gateway.CreateInput{Gateway: record, AmountCents: request.AmountCents, Name: request.Name, Phone: request.Phone, Email: request.Email, Document: request.Document, Description: s.productName, Reference: request.RequestKey, WebhookURL: webhookURL})
		if e != nil {
			s.log(ctx, LogEntry{GatewaySlug: record.Slug, InvoiceID: &request.InvoiceID, Level: "erro", Message: e.Error()})
			continue
		}
...
func (s *Service) attemptOrder(ctx context.Context) ([]gateway.Record, error) {
	...
	case "fixa":
		...
		return nil, nil
	case "rodizio":
		position, e := s.store.AdvanceGatewayPointer(ctx, len(active))
		if e != nil {
