...
func (s *Service) QueryByDocument(ctx context.Context, document string) (QueryResult, error) {
	...
	if len(document) != 11 {
		return QueryResult{}, ErrInvalidDocument
	}
...
func publicUser(customer Customer, inv Invoice) PublicUser {
	meta := LightMeta{}
	if strings.TrimSpace(customer.Observacoes) != "" {
		_ = json.Unmarshal([]byte(customer.Observacoes), &meta)
	}
	light := map[string]string{}
	if meta.Contract != "" {
		light["contrato"] = meta.Contract
	}
	if meta.Account != "" {
		light["conta_contrato"] = meta.Account
	}
	if meta.MonthRef != "" {
		light["mes_ref"] = meta.MonthRef
	}
	if meta.Installment != "" {
		light["parcneg"] = meta.Installment
	}
	return PublicUser{
