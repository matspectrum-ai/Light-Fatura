...
func (s *Service) SignIn(ctx context.Context, email, password string) (Session, error) {
	...
	if session.User.ID == "" || session.AccessToken == "" { return Session{}, ErrInvalidCredentials }
	admin, err := s.store.IsAdmin(ctx, session.User.ID)
	if err != nil { return Session{}, err }
	if !admin { return Session{}, ErrNotAdmin }
	return session, nil
}

func (s *Service) User(ctx context.Context, accessToken string) (User, error) {
	if accessToken == "" { return User{}, ErrNoSession }
	var user User
	if err := s.doJSON(ctx, http.MethodGet, "/auth/v1/user", accessToken, nil, &user); err != nil { return User{}, err }
	return user, nil
}

func (s *Service) Refresh(ctx context.Context, refreshToken string) (Session, error) {
	if refreshToken == "" { return Session{}, ErrNoSession }
	var session Session
	if err := s.doJSON(ctx, http.MethodPost, "/auth/v1/token?grant_type=refresh_token", "", map[string]any{"refresh_token": refreshToken}, &session); err != nil { return Session{}, err }
	if session.AccessToken == "" { return Session{}, ErrNoSession }
	return session, nil
}
...
func (s *Service) RequireAdmin(ctx context.Context, accessToken string) (User, error) {
	...
	if err != nil { return User{}, err }
	if !admin { return User{}, ErrNotAdmin }
	return user, nil
}

func (s *Service) SendRecovery(ctx context.Context, email, redirectTo string) error {
	endpoint := "/auth/v1/recover"
	if redirectTo != "" { endpoint += "?redirect_to=" + url.QueryEscape(redirectTo) }
...
func (s *Service) UpdatePassword(ctx context.Context, accessToken, password string) error {
	if accessToken == "" { return ErrNoSession }
...
func (s *Service) Logout(ctx context.Context, accessToken string) error {
	...
	var api *APIError
	if errors.As(err, &api) && api.Status == http.StatusUnauthorized { return nil }
	return err
}

type APIError struct { Status int; Message string }
func (e *APIError) Error() string { return fmt.Sprintf("supabase auth HTTP %d: %s", e.Status, e.Message) }
...
func (s *Service) doJSON(ctx context.Context, method, endpoint, accessToken string, body any, dst any) error {
	...
	if err != nil { return fmt.Errorf("supabase auth: %w", err) }
	defer resp.Body.Close()
	raw, _ := io.ReadAll(io.LimitReader(resp.Body, 1<<20))
	if resp.StatusCode < 200 || resp.StatusCode >= 300 {
		var message struct { Message string `json:"message"`; ErrorDescription string `json:"error_description"`; Msg string `json:"msg"` }
	...
	if dst != nil && len(raw) > 0 { if err := json.Unmarshal(raw, dst); err != nil { return err } }
	return nil
}

func first(values ...string) string {
	for _, v := range values { if strings.TrimSpace(v) != "" { return v } }
